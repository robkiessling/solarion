/**
 * Battle content records: the stat blocks and scene text battles run on. The engine (lib/battle.js)
 * interprets these; nest definitions (database/pois.js) reference the shared vocabulary by id.
 *
 ...
 *   - database/pois.js       -- the "levels": each nest's difficulty, garrison (bugs), formation, terrain
 *   - database/battle_terrain.js -- the arena obstacle art (ASCII pieces)
 * The formation and terrain-layout ids those records reference (column/ring/surround...,
 * rocks/ruins/canyon) are registries of placement algorithms in lib/battle.js (FORMATIONS,
...
// One-line scene descriptions for the battle footer, assembled by battleBlurb (lib/battle.js): a ground
